...
gpg -d --passphrase "${{secrets.GPG_PASSPHRASE}}" --batch .env.fastlane.android.asc > android/fastlane/.env

# App .env
echo "${{secrets.STAGING_ENV}}" > .env.staging.asc
gpg -d --passphrase "${{secrets.GPG_PASSPHRASE}}" --batch .env.staging.asc > .env.staging
